provider "aws" {
	region = "us-east-1"
    allowed_account_ids = [ "357974209030" ]
}

terraform {
  backend "s3" {
	bucket = "pokescraper-terraform-state"
	key = "terraform.tfstate"
	region = "us-east-1"

	dynamodb_table = "pokescraper-terraform-lock"
	encrypt = true	
  }
}

resource "aws_s3_bucket" "scraper_csv_bucket" {
	bucket = "pokelex-scraper-data"
}

resource "aws_dynamodb_table" "pokelex_data" {
  name = "pokelex_data"
  read_capacity = 1
  write_capacity = 1
  hash_key = "Id"
...
# ONLY DEFINE ATTRIBUTES THAT ARE USED AS HASH/RANGE KEYS
#   attribute {
# 	name = "Title"
# 	type = "S"
#   }
